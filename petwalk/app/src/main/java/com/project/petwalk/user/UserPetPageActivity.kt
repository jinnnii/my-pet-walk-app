...
class UserPetPageActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        // 생성한 temp를 id에 저장
        val id = temp[0]
        binding.tvResult.text = "${id}님의 마이 펫 페이지"
        ...
        binding.btnPetAdd.setOnClickListener(object : View.OnClickListener{
            override fun onClick(p0: View?) {
                ...
                if (animal.isEmpty() || kind.isEmpty() || name.isEmpty()){
                    ...
                    mDialog = ProgressDialog(this@UserPetPageActivity)
                    mDialog.setMessage("등록중입니다...")
                    mDialog.show()
                    val databaseReference = FirebaseDatabase.getInstance().getReference()
                    databaseReference.child("pets").push().setValue(Pet(animal, kind, name))
                    mDialog.dismiss()
                    finish()
                    return
                }
    ...
    private fun uploadToFirebase(uri: Uri) {
        ...
        fileRef.putFile(uri).addOnSuccessListener {
            fileRef.downloadUrl.addOnSuccessListener { uri ->
                ...
                val petImagesUID = databaseRef.child("petImages").push().key
                databaseRef.child("petImages").push().setValue(Images(uri.toString()))
                //프로그래스바 숨김

                databaseRef.child("Users").child(userUID).child("petList").get().addOnSuccessListener {
                    val count = it.childrenCount.toString()
                    databaseRef.child(userUID).child("petList").child(count)
                        .setValue(mapOf(petImagesUID to true))
                }
